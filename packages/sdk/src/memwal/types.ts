...
/** FHE envelope hook surface (impl in PRD-07). Adapter only knows it as an interface. */
export interface FheEnvelope {
  ensureNamespaceKey(
    namespace: string,
  ): Promise<{ keypairId: string; orthogonalMatrixHash: string }>;
  wrap(
    text: string,
    namespace: string,
  ): Promise<{
    ciphertext: Uint8Array;
    blindedVector: number[];
    cofheAttestation: string;
  }>;
  unwrap(
    ciphertext: Uint8Array,
    namespace: string,
  ...
  blindQuery(query: string, namespace: string): Promise<number[]>;
  issuePermit(
    namespace: string,
...
export interface OpenXMemWalConfig {
  ...
  serverUrl?: string;
  /** Optional FHE envelope — when present, all writes pass through CoFHE first. */
  fheEnvelope?: FheEnvelope;
