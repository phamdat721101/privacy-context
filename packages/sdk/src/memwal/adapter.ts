...
export class OpenXMemWalAdapter {
  ...
  async remember(text: string, namespace?: string): Promise<RememberResult> {
    ...
    return this.runOp('remember', { type: 'remember', text, namespace: ns }, async (client) => {
      const wrapped = await this.applyFheEnvelopeText(text, ns);
      const out = client.rememberAndWait
        ? await client.rememberAndWait(wrapped, ns)
        : await client.remember!(wrapped, ns);
  ...
  async recall(
    ...
    return this.runOp('recall', { type: 'recall', query, namespace: ns, limit }, async (client) => {
      const blindedQuery = await this.applyFheEnvelopeQuery(query, ns);
    ...
    for (let i = 0; i < groups.length; i++) {
      ...
      try {
        const wrapped = await Promise.all(
          groups[i].map(async (it) => ({
            text: await this.applyFheEnvelopeText(it.text, it.namespace ?? this.cfg.namespace ?? 'default'),
            namespace: it.namespace ?? this.cfg.namespace,
          })),
        );
        const out = await resilientCall(
          { name: 'memwal.rememberBulk', maxAttempts: 3 },
          () => client.rememberBulkAndWait!(wrapped),
        );
  ...
  // ─── FHE envelope hooks (no-op when fheEnvelope is undefined) ──────────

  private async applyFheEnvelopeText(text: string, namespace: string): Promise<string> {
    if (!this.cfg.fheEnvelope) return text;
    const wrapped = await this.cfg.fheEnvelope.wrap(text, namespace);
    // The relayer stores raw text payloads; we ship a base64-encoded ciphertext
    // so the on-the-wire shape stays a string. The blinded vector is captured
    // by the FheEnvelope impl into its own audit log (see PRD-07 §8).
    return Buffer.from(wrapped.ciphertext).toString('base64');
  }

  private async applyFheEnvelopeQuery(query: string, namespace: string): Promise<string> {
    if (!this.cfg.fheEnvelope) return query;
    const blinded = await this.cfg.fheEnvelope.blindQuery(query, namespace);
    // Encode the blinded vector as a deterministic comma-joined string the
    // relayer treats as opaque text. The recall layer is similarity-based so
    // any unique encoding works; a future enhancement can switch to a binary
    // wire format once the relayer accepts it.
    return blinded.map((n) => n.toFixed(8)).join(',');
  }
