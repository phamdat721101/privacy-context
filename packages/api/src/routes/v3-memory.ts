...
import {
  OpenXMemWalAdapter,
  OpenXMemWalError,
  type MemWalFheEnvelope,
  type MemWalNetwork,
  type OpenXMemWalConfig,
} from '@fhe-ai-context/sdk';
// `createFheEnvelope` lives behind a server-only deep path because it
// imports `node:crypto`. Importing it from the SDK barrel would drag the
// Node-only module into the frontend webpack bundle and break the build.
import { createFheEnvelope } from '@fhe-ai-context/sdk/dist/memwal/fheEnvelope';
...
async function getMemWalAdapter(
  ...
  const adapter = await OpenXMemWalAdapter.create({
    ...
    fheEnvelope: overrides.fheEnvelope,
...
router.post('/remember', requireSuiWallet, async (req: AuthRequest, res) => {
  ...
  try {
    ...
    // audit log via fheEnvelope. Production sets the flag off → real upstream
...
// ─── FHE × MemWal envelope routes (PRD-07 / T29) ─────────────────────────
//
// All four routes Sui-gated AND feature-flagged via FHE_MEMWAL_ENABLE.
// Default off → routes return 503 with an actionable message; non-MemWal
// regression is byte-identical (G3 honored).
//
// Audit log: every wrap / unwrap / blindQuery / permitIssue inserts into
// `memwal_fhe_envelope_logs` (migration 020). NEVER stores plaintext or
// keypair secrets — only object IDs + content hashes.

function fheEnabled(): boolean {
  return process.env.FHE_MEMWAL_ENABLE === 'true';
}

let cachedFheEnvelope: MemWalFheEnvelope | null = null;
function getFheEnvelope(walletAddress: string): MemWalFheEnvelope {
  if (cachedFheEnvelope) return cachedFheEnvelope;
  cachedFheEnvelope = createFheEnvelope({
    ownerSuiAddress: walletAddress,
    masterSecret:
      process.env.FHE_MEMWAL_MASTER_SECRET ??
      // Dev fallback only — production deployments MUST set this env.
      'dev-only-fhe-master-do-not-ship',
  });
  return cachedFheEnvelope;
}

async function logFheEnvelope(args: {
  owner: string;
  namespace: string;
  blob_id?: string | null;
  fhe_keypair_id: string;
  cofhe_tx_hash?: string | null;
  blinded_vector_hash?: string | null;
  op: 'wrap' | 'unwrap' | 'blindQuery' | 'permitIssue';
}): Promise<void> {
  try {
    await pool.query(
      `INSERT INTO memwal_fhe_envelope_logs (
         owner, namespace, blob_id, fhe_keypair_id, cofhe_tx_hash,
         blinded_vector_hash, op
       ) VALUES ($1,$2,$3,$4,$5,$6,$7)`,
      [
        ...
        args.blob_id ?? null,
        args.fhe_keypair_id,
        args.cofhe_tx_hash ?? null,
        args.blinded_vector_hash ?? null,
        args.op,
      ],
    );
  } catch (e) {
    logger.warn(
      { err: (e as Error).message, namespace: args.namespace, op: args.op },
      'memwal:fhe:audit:failed',
    );
  }
}

function rejectWhenDisabled(res: Response): boolean {
  if (fheEnabled()) return false;
  res.status(503).json({
    error: 'fhe_disabled',
    message: 'Set FHE_MEMWAL_ENABLE=true to enable the FHE × MemWal envelope.',
  });
  return true;
}

function vectorHash(v: number[]): string {
  // Hash the first 64 dims as a fingerprint — enough for audit, never inverts.
  const head = v.slice(0, 64).map((n) => n.toFixed(6)).join(',');
  let h = 0;
...
/**
 * POST /v3/memory/fhe/namespace/init — derive + cache the FHE keypair.
 * Body: { namespace: string }
 */
router.post('/fhe/namespace/init', requireSuiWallet, async (req: AuthRequest, res) => {
  if (rejectWhenDisabled(res)) return;
  const wallet = req.user?.address;
  if (!wallet) return res.status(401).json({ error: 'auth required' });
  const { namespace } = (req.body ?? {}) as { namespace?: string };
  if (!namespace) return res.status(400).json({ error: 'namespace required' });
  const env = getFheEnvelope(wallet);
  const r = await env.ensureNamespaceKey(namespace);
  res.json({ ok: true, ...r });
});

/**
 * POST /v3/memory/fhe/remember — wrap text in FHE envelope, then store via adapter.
 * Body: { text: string, namespace: string }
 */
router.post('/fhe/remember', requireSuiWallet, async (req: AuthRequest, res) => {
  if (rejectWhenDisabled(res)) return;
  const wallet = req.user?.address;
  if (!wallet) return res.status(401).json({ error: 'auth required' });
  const { text, namespace } = (req.body ?? {}) as { text?: string; namespace?: string };
  if (!text || !namespace) return res.status(400).json({ error: 'text + namespace required' });
  const accountId = await resolveAccountId(wallet);
  if (!accountId) return res.status(409).json({ error: 'memwal_account_not_provisioned' });

  try {
    const fheEnv = getFheEnvelope(wallet);
    const adapter = await getMemWalAdapter(wallet, accountId, {
      namespace,
      fheEnvelope: fheEnv,
    });
    const out = await adapter.remember(text, namespace);
    const meta = await fheEnv.ensureNamespaceKey(namespace);
    await logFheEnvelope({
      owner: wallet,
      namespace,
      blob_id: out.blob_id ?? null,
      fhe_keypair_id: meta.keypairId,
      blinded_vector_hash: meta.orthogonalMatrixHash,
      op: 'wrap',
    });
    res.json({ ok: true, blob_id: out.blob_id ?? null, job_id: out.job_id ?? null });
  } catch (e) {
...
/**
 * POST /v3/memory/fhe/recall — recall via blinded vector; returns FHE ciphertexts
 * the caller decrypts client-side under their wallet permit.
 * Body: { query: string, namespace: string, limit?: number, minRelevance?: number }
 */
router.post('/fhe/recall', requireSuiWallet, async (req: AuthRequest, res) => {
  if (rejectWhenDisabled(res)) return;
  const wallet = req.user?.address;
  if (!wallet) return res.status(401).json({ error: 'auth required' });
  const { query, namespace, limit, minRelevance } = (req.body ?? {}) as {
    query?: string;
    namespace?: string;
    limit?: number;
    minRelevance?: number;
  };
  if (!query || !namespace) return res.status(400).json({ error: 'query + namespace required' });
  const accountId = await resolveAccountId(wallet);
  if (!accountId) return res.status(409).json({ error: 'memwal_account_not_provisioned' });
  ...
  try {
    ...
    const out = await adapter.recall(query, { limit, namespace, minRelevance });
    const blinded = await fheEnv.blindQuery(query, namespace);
    const meta = await fheEnv.ensureNamespaceKey(namespace);
    await logFheEnvelope({
      owner: wallet,
      namespace,
      fhe_keypair_id: meta.keypairId,
      blinded_vector_hash: vectorHash(blinded),
      op: 'blindQuery',
    });
    // Results carry FHE ciphertexts — caller unwraps client-side under permit.
    res.json({ ok: true, results: out.results, total: out.total, fhe_keypair_id: meta.keypairId });
  } catch (e) {
    sendMemWalError(res, e);
  }
});
...
/**
 * POST /v3/memory/fhe/permit/issue — issue a buyer permit for a namespace.
 * Body: { namespace: string, buyerWallet: string, expirySeconds?: number }
 */
router.post('/fhe/permit/issue', requireSuiWallet, async (req: AuthRequest, res) => {
  if (rejectWhenDisabled(res)) return;
  const wallet = req.user?.address;
  if (!wallet) return res.status(401).json({ error: 'auth required' });
  const { namespace, buyerWallet, expirySeconds } = (req.body ?? {}) as {
    namespace?: string;
    buyerWallet?: string;
    expirySeconds?: number;
  };
  if (!namespace || !buyerWallet)
    return res.status(400).json({ error: 'namespace + buyerWallet required' });

  const fheEnv = getFheEnvelope(wallet);
  const permit = await fheEnv.issuePermit(namespace, buyerWallet as `0x${string}`, expirySeconds ?? 600);
  const meta = await fheEnv.ensureNamespaceKey(namespace);
  await logFheEnvelope({
    owner: wallet,
    namespace,
